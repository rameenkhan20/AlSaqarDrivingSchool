...
export default function dashboard() {
  ...
  const pendingClasses = studentList.filter(item => !completedIds.includes(item.id) && item.scheduledClasses.length > 0)
        ...
        {pendingClasses.map((item) => (
          <Animated.View key={item.id} exiting={SlideOutLeft.duration(300)} layout={LinearTransition.delay(300)} >
            <Schedule
              ...
              studentName={item.studentName}
              timeSlot={item.scheduledClasses[0]?.toLocaleTimeString([], { timeStyle: 'short' }) ?? "Not Scheduled"}
              sessionCompletion={item.sessionCompletion}
              onDone={completedClasses}
            />
          </Animated.View>
